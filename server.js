...
const options = {
    swaggerDefinition: {
        ...
        host: '159.65.235.212:3000', // Your server host
        basePath: '/',
    },
    apis: ['./server.js'], // Ensure this path points to your route definitions
};
...
app.get('/foods', async (req, res) => {
    try {
        ...
        const results = await connection.query(sql); // Execute the SQL query
...
app.get('/custnumber', async (req, res) => {
    try {
        ...
        res.json(formattedResults); // Send the formatted results as JSON
        connection.release();
    } catch (err) {
        console.log('There was an error executing query:', err);
        res.status(500).send('Database error');
